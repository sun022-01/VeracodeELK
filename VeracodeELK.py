...
class XMLAPI:
	...
	def start(self):
		for infos_apps in json.loads(self.getapplist())['applist']['app']:
			...
			try:
				...
				try:
					self.logger.debug('Obtengo los builds.')
					for build_app in json.loads(apps_builds_list)['buildlist']['build']:
						...
						self.logger.debug('Verificando si el build se encuentra en la base de datos.')
						...
						if compare:
							self.logger.debug('Build {0} esta dentro de la base de datos.'.format(build_id))
						else:
	...
	def getapplist(self):
		...
		request = requests.post(self.baseurl+"getapplist.do", auth = RequestsAuthPluginVeracodeHMAC())
		appslist = xmltodict.parse(request.content)

		return json.dumps(appslist)

	def getbuildlist(self, app_id=None, sandbox_id=None):
		if app_id is not None:
			...
			if request.status_code == 200:
				...
				self.logger.error('No se pueden obtener los escaneos de {}. \ Please check manually using Curl, or make sure veracode API service is not down.'.format(app_id))
				exit(0)
	...
	def detailedreport(self, build=None):
		if build is not None:
			if self.output_file.lower() == 'xml':
				...
				report = json.dumps(detailreport)
				datajson = json.loads(report)
				if datajson['detailedreport']['static-analysis']['@submitted_date'] != 0:
					#Modificando submitted date
					submitted_date_r = datajson['detailedreport']['static-analysis']['@submitted_date']
					submitted_date = datetime.strptime(submitted_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
					report = report.replace(submitted_date_r, submitted_date)
					#Modificando published date
					published_date_r = datajson['detailedreport']['static-analysis']['@published_date']
					published_date = datetime.strptime(published_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
					report = report.replace(published_date_r, published_date)
					#Modificando app generation date
					app_generation_date_r = datajson['detailedreport']['@generation_date']
					app_generation_date = datetime.strptime(app_generation_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
					report = report.replace(app_generation_date_r, app_generation_date)
					#Modificando app last update time
					app_last_update_time_r = datajson['detailedreport']['@last_update_time']
					app_last_update_time = datetime.strptime(app_last_update_time_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
					report = report.replace(app_last_update_time_r, app_last_update_time)
					#Modificando app first build submitted date
					app_first_build_submitted_date_r = datajson['detailedreport']['@first_build_submitted_date']
					app_first_build_submitted_date = datetime.strptime(app_first_build_submitted_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
					report = report.replace(app_first_build_submitted_date_r, app_first_build_submitted_date)
				else:	
					#Modificando submitted date
					submitted_date_r = datajson['detailedreport']['dynamic-analysis']['@submitted_date']
					submitted_date = datetime.strptime(submitted_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
					report = report.replace(submitted_date_r, submitted_date)
					#Modificando published date
					published_date_r = datajson['detailedreport']['dynamic-analysis']['@published_date']
					published_date = datetime.strptime(published_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
					report = report.replace(published_date_r, published_date)
					#Modificando app generation date
					app_generation_date_r = datajson['detailedreport']['@generation_date']
					...
					app_first_build_submitted_date = datetime.strptime(app_first_build_submitted_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
					report = report.replace(app_first_build_submitted_date_r, app_first_build_submitted_date)
					#Modificando fechas de vulnerabilidades
				try:
								...
								for categories in vulns['category']:
									try:
										...
										for staticflaws in cwe['staticflaws']['flaw']:
											try:
												date_first_occurrence_r = staticflaws['@date_first_occurrence']
												if search('UTC', date_first_occurrence_r):
													date_first_occurrence = datetime.strptime(date_first_occurrence_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
													report = report.replace(date_first_occurrence_r, date_first_occurrence)
													return report
												grace_period_expires_r = staticflaws['@grace_period_expires']
												if search('UTC', grace_period_expires_r):
													grace_period_expires = datetime.strptime(grace_period_expires_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
													report = report.replace(grace_period_expires_r, grace_period_expires)
													return report
												if  staticflaws['mitigations']['mitigation'] != 0:
													for mitigations in staticflaws['mitigations']['mitigation']:
														date_r = mitigations['@date']
														datem = datetime.strptime(date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
														report = report.replace(date_r, datem)
														return report
												return report
											except:
												pass   
									except:
										categories = vulns['category']
										cwe = categories['cwe']
										staticflaws = cwe['staticflaws']['flaw']
										if search('UTC', date_first_occurrence_r):
											date_first_occurrence = datetime.strptime(date_first_occurrence_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
											report = report.replace(date_first_occurrence_r, date_first_occurrence)
											return report
						...
						for vulns in datajson['detailedreport']['severity']:
							try:
								for categories in vulns['category']:
									try:
										cwe = categories['cwe']
										for dinamicflaws in cwe['dinamicflaws']['flaw']:
											try:
												date_first_occurrence_r = dinamicflaws['@date_first_occurrence']
												if search('UTC', date_first_occurrence_r):
													date_first_occurrence = datetime.strptime(date_first_occurrence_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
													report = report.replace(date_first_occurrence_r, date_first_occurrence)
												grace_period_expires_r = dinamicflaws['@grace_period_expires']
												if search('UTC', grace_period_expires_r):
													grace_period_expires = datetime.strptime(grace_period_expires_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
													report = report.replace(grace_period_expires_r, grace_period_expires)
												if  dinamicflaws['mitigations']['mitigation'] != 0:
													for mitigations in dinamicflaws['mitigations']['mitigation']:
														date_r = mitigations['@date']
														datem = datetime.strptime(date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
														report = report.replace(date_r, datem)
												return report
										...
										categories = vulns['category']
										cwe = categories['cwe']
										dinamicflaws = cwe['dinamicflaws']['flaw']
										if search('UTC', date_first_occurrence_r):
											date_first_occurrence = datetime.strptime(date_first_occurrence_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
											report = report.replace(date_first_occurrence_r, date_first_occurrence)
										grace_period_expires_r = dinamicflaws['@grace_period_expires']
								...
								try:
									...
									categories = vulns['category']
									cwe = categories['cwe']
									staticflaws = cwe['dinamicflaws']['flaw']
									if search('UTC', date_first_occurrence_r):
										date_first_occurrence = datetime.strptime(date_first_occurrence_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
										report = report.replace(date_first_occurrence_r, date_first_occurrence)
										return report
					...
					try:
						sca = datajson['detailedreport']['software_composition_analysis']
						for components in sca['vulnerable_components']['component']:
							try:
								added_date_r = components['@added_date']
								if search('UTC', added_date_r):
									added_date = datetime.strptime(added_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
									report = report.replace(added_date_r, added_date)
								return report
							except:
								return report
						return report
						...
						for components in sca['vulnerable_components']['component']:
							try:
								vulnerability = components['vulnerabilities']['vulnerability']
								first_found_date_r = vulnerability['@first_found_date']
								if search('UTC', first_found_date_r):
									first_found_date = datetime.strptime(first_found_date_r, '%Y-%m-%d %H:%M:%S UTC').isoformat()
									report = report.replace(first_found_date_r, first_found_date)
									return report
								return report
							except:
								return report
	...
	def clearreport(self, report=None):
		if report is not None:
			...
			self.logger.debug('# Obtendo informaciones de flaw status.')
			...
			for vulns in datajson['detailedreport']:
				if len(vulns) > 1:
					if len(vulns['category']) == 6:
						...
						try:
							for categories in vulns['category']:
								...
								try:
									...
									self.logger.debug("6: {e} no encontrado, cambiado por null".format(e=e))
						...
						for categories in vulns['category']:
							...
							try:
								...
								self.logger.debug("{e} no encontrado, cambiado por null".format(e=e))
								desc_list.append(categories['desc']['para']['@text'])
...
class veracode:
	def __init__(self):
		...
		self.logger = logging.getLogger('Iniciando API Veracode XML')
	...
	def start(self):
		...
		self.logger.info('Tomando informacion de config.yml.')
		...
		if args.output.lower() in files:
			...
			self.logger.info('Verificando directorios.')
			...
			self.logger.info('Comenzando proceso de descarga de escaneos.')
			API = XMLAPI(
			...
			try:
				...
				self.logger.error('Credenciales invalidas, configurarlas en el archivo "config.yml".')
		else:
			self.logger.error('Ha introducido un formato de salida que no se puede reproducir, por favor vuelva a intentarlo seleccionando entre "xml", "json" y "csv".')
			exit(0)
try:
